...
async function allFollowers(username) {
    try {
        ...
        if (!response.ok) {
            throw new Error(chalk.red(`Failed to fetch followers data: ${response.status}`));
        }
        const followersData = await response.json();
        const followerUsernames = followersData.map((follower) => follower.login);
                ...
                const { followerName } = await inquirer.prompt({
                    ...
                    validate: function (input) {
                        if (followerUsernames.includes(input)) {
                            ...
                            return chalk.red("Invalid username. Please enter a valid username.");
                        }
                    },
                });
...
async function allFollowing(username) {
    try {
        const response = await fetch(`https://api.github.com/users/${username}/following`);
        if (!response.ok) {
            throw new Error(chalk.red(`Failed to fetch following data: ${response.status}`));
        }
        const followingData = await response.json();
        const followingUsernames = followingData.map((following) => following.login);
...
async function viewFollowerDetails(followerName) {
    try {
        const response = await fetch(`https://api.github.com/users/${followerName}`);
        if (!response.ok) {
            throw new Error(chalk.red(`Failed to fetch data for follower ${followerName}: ${response.status}`));
        }
        ...
        console.log(chalk.magenta(`First repository created at: ${followerDetails.created_at}`));
        console.log(chalk.magenta(`Last repository updated at: ${followerDetails.updated_at}`));
        console.log(chalk.yellow("----------------------------------------------------"));
    }
    catch (error) {
        console.error(chalk.red(`Error fetching data for follower ${followerName}:`), error);
    }
}
async function viewFollowingDetails(followingName) {
    try {
        const response = await fetch(`https://api.github.com/users/${followingName}`);
        if (!response.ok) {
            throw new Error(chalk.red(`Failed to fetch data for user ${followingName}: ${response.status}`));
        }
        ...
        console.log(chalk.magenta(`First repository created at: ${followingDetails.created_at}`));
        console.log(chalk.magenta(`Last repository updated at: ${followingDetails.updated_at}`));
        console.log(chalk.yellow("----------------------------------------------------"));
    }
    catch (error) {
        console.error(chalk.red(`Error fetching data for user ${followingName}:`), error);
    }
}
main();
